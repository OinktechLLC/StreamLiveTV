# StreamLiveTV Creator

Удобная платформа для запуска собственных онлайн-каналов (ТВ/радио), управления эфиром, общения с аудиторией и публикации встраиваемого плеера.

## Что это за платформа

**StreamLiveTV Creator** — это веб‑приложение, где можно:
- создавать каналы и запускать 24/7 эфир;
- подключать разные источники контента (например, YouTube, MP4, M3U8);
- смотреть каналы через встроенный плеер;
- общаться в чате и развивать сообщество;
- использовать профиль, поиск, API и историю обновлений.

Основные разделы приложения:
- `/` — лендинг;
- `/browse` — каталог каналов;
- `/create-channel` — создание канала;
- `/channel/:id` — страница канала;
- `/embed/:id` и `/popout/:id` — варианты плеера;
- `/api-docs` — документация API.

---

## Быстрый старт (локально)

### 1) Требования
- Node.js 18+
- npm 9+

### 2) Установка и запуск

```bash
npm install
npm run dev
```

После запуска откройте адрес из терминала (обычно `http://localhost:5173`).

### 3) Полезные команды

```bash
npm run dev      # режим разработки
npm run build    # production-сборка
npm run preview  # локальный просмотр production-сборки
npm run lint     # проверка ESLint
```

---

## Технологии

- React 18 + TypeScript
- Vite
- Tailwind CSS + shadcn/ui
- React Router
- TanStack Query
- Supabase

---

## Деплой и резервные сайты

Ниже рекомендуемая схема публикации:

- **Основной сайт**: Vercel / Cloudflare Pages
- **Резервный сайт (fallback)**: Netlify

Это удобно: если у одного провайдера временные проблемы, можно быстро переключить трафик на второй.

### Рекомендуемые URL (заполните своими доменами)

- Основной (Vercel): `https://<your-project>.vercel.app`
- Резервный (Netlify): `https://<your-project>.netlify.app`

> Если есть собственный домен, настройте `A/CNAME` и резервный поддомен, например:
> - `app.example.com` → Vercel
> - `backup.example.com` → Netlify

### Минимальные настройки деплоя

Для обоих сервисов:
- **Build command**: `npm run build`
- **Publish directory**: `dist`
- **Node version**: 18+
- Добавьте одинаковые переменные окружения (особенно Supabase), чтобы основной и резервный сайты работали одинаково.

### Cloudflare Pages (если деплоите туда)

- **Framework preset**: `Vite`
- **Install command**: `npm install`
- **Build command**: `npm run build`
- **Output directory**: `dist`

> В этом проекте может отсутствовать актуальный npm lock-файл, поэтому для Cloudflare Pages используйте `npm install`, а не `npm ci`.

### Если сборка в Cloudflare Pages падает

Проверьте в **Deployments → конкретный деплой → View build log**:
- какой Node/NPM используется;
- не установлена ли переменная `NODE_ENV=production` (в таком случае dev-зависимости могут не ставиться);
- что команды выставлены так: `npm install` → `npm run build` → output `dist`.

В репозитории зафиксирован `wrangler.toml` с безопасной `compatibility_date` (не в будущем), чтобы Cloudflare не отклонял деплой на этапе конфигурации.

### Если видите Cloudflare Error 522 (Connection timed out)

`522` означает, что Cloudflare не получил ответ от origin-сервера. Для **Cloudflare Pages** это обычно связано не с фронтенд-кодом, а с привязкой домена/маршрутизацией:

1. Откройте **Cloudflare Dashboard → Pages → ваш проект → Custom domains**.
2. Проверьте, что домен действительно привязан к нужному Pages‑проекту и прошёл проверку.
3. В **DNS** убедитесь, что нет конфликтующих `A/AAAA/CNAME` записей для того же хоста.
4. Если используете внешний origin (VPS/Nginx/другой хостинг) под тем же доменом, проверьте, что сервер доступен на `80/443` и отвечает из интернета.
5. Временно проверьте доступность через `https://<project>.pages.dev`:
   - если `pages.dev` открывается, а кастомный домен нет — проблема в DNS/домене;
   - если не открывается даже `pages.dev` — проверьте статус последнего деплоя и логи Pages.

Полезно также отключить проксирование (серое облако) для проблемной записи в DNS на время диагностики, чтобы понять, проблема в Cloudflare‑прокси или в самом origin.

---

## Переменные окружения

Создайте `.env` (или настройте переменные в панели Vercel/Netlify):

```bash
VITE_SUPABASE_URL=...
VITE_SUPABASE_ANON_KEY=...
```

> Никогда не коммитьте секреты в репозиторий.

---

## Структура проекта (кратко)

- `src/pages` — страницы приложения
- `src/components` — UI и бизнес-компоненты
- `src/integrations` — интеграции (например, Supabase)
- `public` — статические файлы

---

## Для пользователей платформы

1. Зарегистрируйтесь/войдите.
2. Создайте канал.
3. Добавьте источник трансляции.
4. Откройте страницу канала и проверьте эфир.
5. При необходимости используйте embed/popout‑плеер для внешних сайтов.

---

## Лицензия и права

© 2024–2026 StreamLiveTV / TOO Oink Tech Ltd Co. Все права защищены.
